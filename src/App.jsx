...
function processTransfers(transfers) {
  const p = transfers.map((tx) => ({ hash: tx.transaction_hash, from: tx.from_address, value: parseFloat((parseInt(tx.value) / 1e6).toFixed(2)), timestamp: new Date(tx.block_timestamp) })).sort((a, b) => b.timestamp - a.timestamp);
      ...
      <div className="card" style={{ width: '100%', maxWidth: 580, maxHeight: '90vh', overflowY: 'auto', padding: 28 }}>
        ...
        {!adminUnlocked ? (
          <div style={{ textAlign: 'center', padding: '20px 0' }}>
            ...
            <input autoFocus type="password" value={pinInput} onChange={(e) => setPinInput(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handlePinSubmit()} placeholder="••••" maxLength={8} style={{ background: '#0f172a', border: '1px solid ' + (pinError ? '#ef4444' : '#334155'), borderRadius: 8, padding: '12px 16px', color: '#f1f5f9', fontSize: '1.2em', textAlign: 'center', width: 160, letterSpacing: '0.3em', outline: 'none', fontFamily: 'monospace' }} />
          ...
          <div>
            <div style={{ marginBottom: 24 }}>
              <div style={{ fontSize: '0.78em', color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 14 }}>Safes Actuales ({safes.length})</div>
              {safes.map((safe, idx) => (
                <div key={idx} style={{ background: '#0f172a', borderRadius: 10, padding: '12px 14px', marginBottom: 8, borderLeft: '3px solid ' + COLOR_PALETTE[idx % COLOR_PALETTE.length] }}>
                  {editIdx === idx ? (
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
                      <span style={{ color: COLOR_PALETTE[idx % COLOR_PALETTE.length], fontWeight: 700, fontSize: '0.9em', minWidth: 56 }}>{safe.name}</span>
                      <span style={{ color: '#475569', fontSize: '0.73em', flex: 1, wordBreak: 'break-all' }}>{safe.address}</span>
                      <div style={{ display: 'flex', gap: 6, flexShrink: 0 }}>
                        <button onClick={() => { setEditIdx(idx); setEditName(safe.name); setEditAddr(safe.address); }} style={{ background: '#1e293b', color: '#818cf8', border: '1px solid #334155', borderRadius: 6, padding: '4px 10px', cursor: 'pointer', fontSize: '0.75em', fontFamily: 'monospace' }}>✏️ Editar</button>
                        <button onClick={() => handleDeleteSafe(idx)} disabled={saving} style={{ background: '#1e293b', color: '#ef4444', border: '1px solid #334155', borderRadius: 6, padding: '4px 10px', cursor: 'pointer', fontSize: '0.75em', fontFamily: 'monospace' }}>🗑 Eliminar</button>
                      </div>
                    </div>
                  )}
                </div>
...
export default function App() {
  ...
  const refreshFiat = async () => {
    ...
    if (sheetData) { setFiatData(sheetData); setFiatSource('sheet'); setFiatLastUpdate(new Date()); }
  ...
  useEffect(() => {
    ...
    refreshFiat();
    const iv = setInterval(refreshFiat, 60 * 60 * 1000);
  ...
  const handleEditSave = async (idx) => {
    ...
    await persistSafes(safes.map((s, i) => i === idx ? { name: editName.trim().toUpperCase(), address: editAddr } : s));
    setEditIdx(null);
  };
    ...
    <div style={{ minHeight: '100vh', background: '#0f172a', color: '#e2e8f0', fontFamily: 'monospace' }}>
      <style>{`
        * { box-sizing: border-box; }
        ...
        .tab { padding: 8px 18px; border-radius: 8px; cursor: pointer; font-size: 0.85em; font-weight: 600; border: none; fontFamily: monospace; }
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: '0.78em', color: '#64748b' }}>
              Polygon · {safes.length} Safes · Fiat + Crypto
              {storageError ? <span style={{ color: '#fb923c', marginLeft: 8 }}>· ⚠️ Error sync</span> : <span style={{ color: '#00d4aa', marginLeft: 8 }}>· 🌐 JSONbin sync</span>}
              {fiatSource === 'sheet' && fiatLastUpdate && <span style={{ color: '#00d4aa', marginLeft: 8 }}>· 📊 Sheet {fiatLastUpdate.toLocaleTimeString()}</span>}
              {fiatSource === 'fallback' && <span style={{ color: '#fb923c', marginLeft: 8 }}>· ⚠️ Fiat: datos locales</span>}
            </div>
          </div>
        </div>
      ...
      <div style={{ padding: '24px', maxWidth: 1400, margin: '0 auto' }}>
        {!data && !loading && (
          <div className="fade" style={{ maxWidth: 520, margin: '60px auto', textAlign: 'center' }}>
            ...
            <p style={{ color: '#64748b', fontSize: '0.95em', lineHeight: 1.8, marginBottom: 32 }}>Cargará balances USDT on-chain + datos fiat {fiatSource === 'sheet' ? 'desde Google Sheets ✅' : 'locales ⚠️'} para {safes.length} Safes.</p>
            <div className="card" style={{ padding: '14px 18px', marginBottom: 28, textAlign: 'left' }}>
              {safes.map((s, i) => (
                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 0', borderBottom: i < safes.length - 1 ? '1px solid #334155' : 'none' }}>
                  <div style={{ width: 7, height: 7, borderRadius: '50%', background: COLOR_PALETTE[i % COLOR_PALETTE.length], flexShrink: 0 }} />
                  <span style={{ color: COLOR_PALETTE[i % COLOR_PALETTE.length], fontWeight: 700, fontSize: '0.9em', minWidth: 56 }}>{s.name}</span>
                  <span style={{ color: '#475569', fontSize: '0.75em', wordBreak: 'break-all' }}>{s.address}</span>
                </div>
              ))}
            </div>
            ...
            <p style={{ color: '#334155', fontSize: '0.75em', marginTop: 12 }}>Tiempo estimado: {safes.length * 10}–{safes.length * 15} segundos</p>
          </div>
        )}
        ...
        {data && !loading && (
          <div className="fade">
              ...
              return (
                <div>
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(200px,1fr))', gap: 12, marginBottom: 20 }}>
                    ...
                    <KPI label="Total Inversores" value={fTotals.inversores.toLocaleString()} sub={cTotals.txs + ' TXs blockchain'} color="#f472b6" />
                  </div>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(300px,1fr))', gap: 12 }}>
                    {data.map((safe, idx) => {
                      ...
                      return (
                        <div key={idx} className="card" style={{ padding: 20, borderTop: '3px solid ' + COLORS[idx] }}>
                          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 14 }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}><div style={{ width: 9, height: 9, borderRadius: '50%', background: COLORS[idx] }} /><span style={{ fontWeight: 700, color: '#f1f5f9', fontSize: '1.05em' }}>{safe.name}</span></div>
                            <div style={{ textAlign: 'right' }}><div style={{ fontSize: '1.2em', fontWeight: 700, color: COLORS[idx] }}>{fmt(safeTotalUSD)}</div><div style={{ fontSize: '0.75em', color: '#475569' }}>{pct(totalUSD > 0 ? (safeTotalUSD / totalUSD) * 100 : 0)} del total</div></div>
                          </div>
                          <div style={{ display: 'flex', borderRadius: 4, overflow: 'hidden', height: 20, marginBottom: 12 }}>
                            ...
                            <div style={{ background: '#fb923c', width: pct(safePctCripto), display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.65em', fontWeight: 700, color: '#0f172a' }}>{safePctCripto > 15 ? pct(safePctCripto) : ''}</div>
                          </div>
                          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                            {[{ label: '🏦 Fiat USD', value: fmt(safeFiatUSD), sub: (safe.fiat?.count || 0) + ' inversores · ' + pct(safePctFiat), color: '#818cf8' }, { label: '🔗 Cripto USD', value: fmt(safeCriptoUSD), sub: safe.metrics.count + ' txs · ' + pct(safePctCripto), color: '#fb923c' }].map((m, i) => (
            ...
            {activeTab === 'cripto' && (
              <div>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(200px,1fr))', gap: 12, marginBottom: 20 }}>
                  ...
                  <KPI label={`Promedio / ${safes.length} Safes`} value={fmt(parseFloat(cTotals.received) / safes.length)} sub="USDT recibido" color="#f472b6" />
                </div>
                  ...
                  {data.map((safe, idx) => (
                    <div key={idx} className="card" style={{ padding: 20, borderTop: '3px solid ' + COLORS[idx] }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16, gap: 10 }}>
                        <div>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 5 }}><div style={{ width: 8, height: 8, borderRadius: '50%', background: COLORS[idx] }} /><span style={{ fontWeight: 700, color: '#f1f5f9', fontSize: '1.05em' }}>{safe.name}</span><span style={{ fontSize: '0.65em', padding: '2px 6px', borderRadius: 4, background: COLORS[idx] + '22', color: COLORS[idx], fontWeight: 600 }}>POLYGON</span></div>
                          <div style={{ fontSize: '0.72em', color: '#475569', wordBreak: 'break-all' }}>{safe.address}</div>
                        </div>
                        <a href={'https://polygonscan.com/address/' + safe.address} target="_blank" rel="noreferrer" style={{ background: '#0f172a', color: COLORS[idx], padding: '5px 11px', borderRadius: 6, textDecoration: 'none', fontSize: '0.75em', fontWeight: 600, whiteSpace: 'nowrap', border: '1px solid ' + COLORS[idx] + '44', flexShrink: 0 }}>Polygonscan ↗</a>
                      </div>
                      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 8, marginBottom: 16 }}>
                        {[{ label: 'Balance', value: fmt(safe.balance), color: COLORS[idx] }, { label: 'Recibido', value: fmt(safe.metrics.total), color: '#00d4aa' }, { label: 'TXs', value: safe.metrics.count.toLocaleString(), color: '#94a3b8' }, { label: 'Promedio', value: fmt(safe.metrics.average), color: '#64748b' }].map((m, i) => (
                          <div key={i} style={{ background: '#0f172a', borderRadius: 8, padding: '11px 8px', textAlign: 'center' }}><div style={{ fontSize: '0.72em', color: '#475569', textTransform: 'uppercase', marginBottom: 5 }}>{m.label}</div><div style={{ fontSize: '1em', fontWeight: 700, color: m.color }}>{m.value}</div></div>
                        ))}
                      </div>
                    ...
                    return (
                      <div key={idx} className="card" style={{ padding: 20, borderTop: '3px solid ' + COLORS[idx] }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}><div style={{ width: 9, height: 9, borderRadius: '50%', background: COLORS[idx] }} /><span style={{ fontWeight: 700, color: '#f1f5f9', fontSize: '1.05em' }}>{safe.name}</span></div>
                          <div style={{ textAlign: 'right' }}><div style={{ color: COLORS[idx], fontWeight: 700, fontSize: '1.2em' }}>{fmt(fiat.totalUsd)}</div><div style={{ fontSize: '0.72em', color: '#475569' }}>€{fiat.totalEur.toLocaleString('es-ES',{maximumFractionDigits:0})}</div></div>
                        </div>
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 16 }}>
                          {[{ label: 'Inversores', value: fiat.count, color: COLORS[idx] }, { label: 'Promedio', value: fmt(fiat.count > 0 ? fiat.totalUsd / fiat.count : 0), color: '#94a3b8' }].map((m, i) => (
                            <div key={i} style={{ background: '#0f172a', borderRadius: 8, padding: '11px 8px', textAlign: 'center' }}><div style={{ fontSize: '0.72em', color: '#475569', textTransform: 'uppercase', marginBottom: 5 }}>{m.label}</div><div style={{ fontSize: i===0?'1.4em':'1.1em', fontWeight: 700, color: m.color }}>{m.value}</div></div>
                          ))}
                        </div>
                        {fiat.txs.length > 0 ? (
                          <details>
            ...
            <div style={{ marginTop: 32, textAlign: 'center', color: '#1e293b', fontSize: '0.72em' }}>
              Raised Capital · Polygon · Moralis API · {safes.length} Safes · {fiatSource === 'sheet' ? '📊 Fiat desde Google Sheets' : '⚠️ Fiat local'} · 🌐 JSONbin sync
            </div>
          </div>
        )}
      </div>
