...
async function getBalance(address) {
  try {
    ...
    const total = data
      .filter(t => STABLECOINS.includes(t.token_address?.toLowerCase()))
...
async function getTransfers(address) {
  try {
    let all = [];
    for (const contract of STABLECOINS) {
      let cursor = null, pages = 0;
      while (pages < 20) {
        const url = `https://deep-index.moralis.io/api/v2.2/${address}/erc20/transfers?chain=0x89&contract_addresses[]=${contract}&limit=100${cursor ? '&cursor=' + cursor : ''}`;
