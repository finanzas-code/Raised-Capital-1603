...
function LoginScreen({ onLogin }) {
  ...
  const handleLogin = () => {
    if (user === LOGIN_USER && pass === LOGIN_PASS) {
      sessionStorage.setItem('rc_auth', 'ok');
      onLogin();
    } else {
  ...
  return (
    <div style={{ minHeight:'100vh', background:'#0f172a', display:'flex', alignItems:'center', justifyContent:'center', fontFamily:'monospace' }}>
      <div style={{ background:'#1e293b', border:'1px solid #334155', borderRadius:16, padding:'40px 36px', width:'100%', maxWidth:380, textAlign:'center' }}>
        <div style={{ width:52, height:52, borderRadius:12, background:'linear-gradient(135deg,#00d4aa,#818cf8)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:26, fontWeight:800, margin:'0 auto 20px', color:'#0f172a' }}>R</div>
        <div style={{ fontWeight:800, color:'#f1f5f9', fontSize:'1.3em', marginBottom:4 }}>Raised Capital</div>
        <div style={{ color:'#475569', fontSize:'0.8em', marginBottom:28 }}>Acceso restringido</div>
        <div style={{ textAlign:'left', marginBottom:12 }}>
          <div style={{ fontSize:'0.75em', color:'#64748b', marginBottom:5 }}>Usuario</div>
          <input autoFocus value={user} onChange={e=>setUser(e.target.value)} onKeyDown={e=>e.key==='Enter'&&document.getElementById('rc_pass').focus()} placeholder="usuario"
            style={{ width:'100%', background:'#0f172a', border:'1px solid #334155', borderRadius:8, padding:'10px 14px', color:'#f1f5f9', fontSize:'0.95em', fontFamily:'monospace', outline:'none' }}/>
        </div>
        <div style={{ textAlign:'left', marginBottom:20 }}>
          <div style={{ fontSize:'0.75em', color:'#64748b', marginBottom:5 }}>Contraseña</div>
          <input id="rc_pass" type="password" value={pass} onChange={e=>setPass(e.target.value)} onKeyDown={e=>e.key==='Enter'&&handleLogin()} placeholder="••••••••"
            style={{ width:'100%', background:'#0f172a', border:`1px solid ${error?'#ef4444':'#334155'}`, borderRadius:8, padding:'10px 14px', color:'#f1f5f9', fontSize:'0.95em', fontFamily:'monospace', outline:'none' }}/>
        </div>
        {error && <div style={{ color:'#ef4444', fontSize:'0.8em', marginBottom:14 }}>⚠️ {error}</div>}
        <button onClick={handleLogin} style={{ width:'100%', background:'#00d4aa', color:'#0f172a', border:'none', borderRadius:9, padding:'12px', fontSize:'1em', fontWeight:700, cursor:'pointer', fontFamily:'monospace' }}>
          Entrar →
        </button>
      </div>
    </div>
      ...
      <div className="card" style={{ width:'100%', maxWidth:640, maxHeight:'90vh', overflowY:'auto', padding:28 }}>
        ...
        {!hasData && adminUnlocked && (
          <div style={{ background:'#f59e0b18', border:'1px solid #f59e0b44', borderRadius:8, padding:'8px 14px', marginBottom:16, fontSize:'0.78em', color:'#f59e0b' }}>
            ⚠️ Para marcar Sold Out, primero carga el dashboard
          </div>
        )}
        {!adminUnlocked ? (
                  ...
                  {editIdx === idx ? (
                    ...
                    <div style={{ display:'flex', alignItems:'center', gap:8, flexWrap:'wrap' }}>
                      ...
                      <div style={{ display:'flex', gap:5, flexShrink:0, marginLeft:'auto' }}>
                        ...
                        <button onClick={()=>handleToggleSoldOut(idx)} disabled={saving||(!safe.soldOut&&!hasData)}
                          ...
                          {safe.soldOut?'▶ Reactivar':'🔒 Sold Out'}
                        </button>
...
// ─────────────────────────────────────────────────────────────────────────────
// APP — enruta entre login y dashboard
// ─────────────────────────────────────────────────────────────────────────────
export default function App() {
  const [loggedIn, setLoggedIn] = useState(() => sessionStorage.getItem('rc_auth') === 'ok');
...
function Dashboard() {
  ...
  const loadAll = async () => {
    ...
    try {
      ...
      if (sheetData) { Object.keys(sheetData).forEach(k => { fiatActual[k] = sheetData[k]; }); setFiatData(fiatActual); setFiatSource('sheet'); setFiatLastUpdate(new Date()); }
      const results = [];
      for (const safe of safes) {
        if (safe.soldOut && safe.frozenData) {
          setMsg(`${safe.name} (SOLD OUT)`);
          results.push({ ...safe, balance: safe.frozenData.balance||'0.00', transfers: [], metrics: { total:(safe.frozenData.criptoUsd||0).toFixed(2), count:safe.frozenData.criptoTxs||0, average:'0.00' }, fiat: { totalEur:safe.frozenData.fiatEur||0, totalUsd:safe.frozenData.fiatUsd||0, count:safe.frozenData.inversores||0, txs:[] } });
        } else {
  ...
  const handleAddSafe = async () => {
    ...
    if (!newSafeName.trim()) return setAddError('Nombre vacío');
    if (!isValidAddress(newSafeAddr)) return setAddError('Dirección inválida');
    if (safes.find(s=>s.address.toLowerCase()===newSafeAddr.toLowerCase())) return setAddError('Dirección ya existe');
    if (safes.find(s=>s.name.toLowerCase()===newSafeName.toLowerCase())) return setAddError('Nombre ya existe');
    ...
    setAddSuccess('Safe añadido ✅'); setTimeout(()=>setAddSuccess(''), 3000);
    setData(null);
  };

  const handleDeleteSafe = async (idx) => {
    if (!window.confirm('¿Eliminar "'+safes[idx].name+'"?')) return;
    await persistSafes(safes.filter((_,i)=>i!==idx)); setData(null);
  };
  ...
  const handleResetDefaults = async () => {
    if (!window.confirm('¿Restaurar Safes por defecto?')) return;
    await persistSafes(DEFAULT_SAFES);
    setAddSuccess('Restaurados ✅'); setTimeout(()=>setAddSuccess(''), 3000); setData(null);
  };

  const handleToggleSoldOut = async (idx) => {
    const safe = safes[idx];
    if (safe.soldOut) {
      if (!window.confirm(`¿Reactivar "${safe.name}"?`)) return;
      const updated = safes.map((s,i) => { if (i!==idx) return s; const {soldOut,frozenData,...rest}=s; return rest; });
      await persistSafes(updated); setData(null); return;
    }
    if (!data) return;
    const safeData = data.find(d=>d.name===safe.name);
    ...
    const criptoUsd2026 = safeData?(safeData.transfers||[]).filter(tx=>tx.timestamp.getFullYear()===2026).reduce((s,tx)=>s+tx.value,0):0;
    const criptoTxs2026 = safeData?(safeData.transfers||[]).filter(tx=>tx.timestamp.getFullYear()===2026).length:0;
    const fiatUsd2026 = (fiatSafe.txs||[]).filter(tx=>getYearFromFecha(tx.fecha)==='2026').reduce((s,tx)=>s+(tx.importe||0)*EUR_USD,0);
    const fiatCount2026 = (fiatSafe.txs||[]).filter(tx=>getYearFromFecha(tx.fecha)==='2026').length;
    if (!window.confirm(`¿Marcar "${safe.name}" como SOLD OUT?\nTotal: ${fmt(totalUsd)}\nDejará de consultar blockchain y sheet.`)) return;
    const updated = safes.map((s,i)=>i!==idx?s:{...s,soldOut:true,frozenData:{fiatEur:fiatSafe.totalEur,fiatUsd:fiatSafe.totalUsd,inversores:fiatSafe.count,criptoUsd,criptoTxs,totalUsd,balance,date:today,criptoUsd2026,criptoTxs2026,fiatUsd2026,fiatCount2026,total2026:criptoUsd2026+fiatUsd2026}});
    await persistSafes(updated);
  ...
  const fTotals = data
    ? { totalUsd:data.reduce((s,d)=>s+(d.fiat?.totalUsd||0),0), totalEur:data.reduce((s,d)=>s+(d.fiat?.totalEur||0),0), inversores:data.reduce((s,d)=>s+(d.fiat?.count||0),0) }
    : { totalUsd:Object.values(fiatData).reduce((s,d)=>s+(d.totalUsd||0),0), totalEur:Object.values(fiatData).reduce((s,d)=>s+(d.totalEur||0),0), inversores:Object.values(fiatData).reduce((s,d)=>s+(d.count||0),0) };
  ...
  const BudgetSection = () => {
    ...
    const mFiat = new Array(13).fill(0), mCripto = new Array(13).fill(0);
    (data||[]).forEach(safe => {
      if (safe.soldOut && safe.frozenData) {
        // FIAT sold out: usar fiatData con txs reales por fecha (distribuye correctamente por mes)
        const sfSO = fiatData[safe.name];
        if (sfSO && sfSO.txs && sfSO.txs.length > 0) {
          (sfSO.txs||[]).forEach(tx=>{try{const f=String(tx.fecha||'');let yr,mo;if(f.includes('/')){const p=f.split('/');yr=p[2];mo=parseInt(p[1]);}else{yr=f.slice(0,4);mo=parseInt(f.slice(5,7));}if(yr==='2026'&&mo>=1&&mo<=12)mFiat[mo]+=(tx.importe||0)*EUR_USD;}catch(e){}});
        } else if (safe.frozenData.fiatUsd2026!=null) {
          // Fallback: sin txs, imputar al mes de cierre
        ...
        // CRIPTO sold out: sin desglose mensual real, imputar al mes de cierre
        ...
        (safe.transfers||[]).forEach(tx=>{try{if(tx.timestamp.getFullYear()===2026)mCripto[tx.timestamp.getMonth()+1]+=tx.value||0;}catch(e){}});
      }
    });
    ...
    return (
      <div className="card" style={{padding:20,marginTop:20}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(160px,1fr))',gap:10,marginBottom:20}}>
          {[{label:'Real YTD',value:fmt(ytdReal),color:'#00d4aa',sub:'Captado en 2026'},{label:'Goal YTD',value:fmt(ytdGoal),color:'#818cf8',sub:'Objetivo acumulado'},{label:'% YTD',value:ytdPct.toFixed(1)+'%',color:ytdPct>=100?'#00d4aa':ytdPct>=80?'#fb923c':'#ef4444',sub:ytdPct>=100?'✅ On track':'⚠️ Por debajo'},{label:'Gap YTD',value:(ytdGap<=0?'+':'-')+fmt(Math.abs(ytdGap)),color:ytdGap<=0?'#00d4aa':'#f472b6',sub:ytdGap<=0?'Superado':'Pendiente'}].map((k,i)=>(
            <div key={i} style={{background:'#0f172a',borderRadius:10,padding:'14px 16px',borderLeft:'3px solid '+k.color}}>
              <div style={{fontSize:'0.72em',color:'#64748b',textTransform:'uppercase',marginBottom:6}}>{k.label}</div>
        ...
        <ResponsiveContainer width="100%" height={260}>
          <BarChart data={rows} barGap={4} margin={{top:5,right:10,left:10,bottom:5}}>
            ...
            <Bar dataKey="real" name="real" radius={[4,4,0,0]}>{rows.map((r,i)=><Cell key={i} fill={r.real===null?'transparent':r.onTrack?'#00d4aa':'#fb923c'}/>)}</Bar>
          </BarChart>
        </ResponsiveContainer>
        <table style={{width:'100%',borderCollapse:'collapse',fontSize:'0.84em',marginTop:16}}>
          <thead><tr style={{color:'#475569',fontSize:'0.82em',textTransform:'uppercase'}}>{['Mes','Goal Mes','Real Mes','% Cumpl.','Gap','Estado'].map((h,i)=><th key={i} style={{padding:'7px 10px',textAlign:i>=2?'right':'left',fontWeight:500,borderBottom:'1px solid #1e293b'}}>{h}</th>)}</tr></thead>
          <tbody>{rows.map((r,i)=>(
  ...
  return (
    <div style={{minHeight:'100vh',background:'#0f172a',color:'#e2e8f0',fontFamily:'monospace'}}>
      ...
      {fiatLoading && (
        <div style={{position:'fixed',bottom:24,right:24,zIndex:50,background:'#1e293b',border:'1px solid #00d4aa44',borderRadius:12,padding:'12px 20px',display:'flex',alignItems:'center',gap:12,boxShadow:'0 4px 24px #00000066'}}>
          <div style={{width:18,height:18,border:'2px solid #1e293b',borderTop:'2px solid #00d4aa',borderRadius:'50%',animation:'spin 0.8s linear infinite',flexShrink:0}}/>
          <div>
            <div style={{color:'#00d4aa',fontWeight:700,fontSize:'0.85em'}}>Cargando datos fiat...</div>
            <div style={{color:'#475569',fontSize:'0.72em',marginTop:2}}>Sincronizando Google Sheets</div>
          </div>
        </div>
      )}
        ...
        {!data&&!loading&&(
          <div className="fade" style={{maxWidth:520,margin:'60px auto',textAlign:'center'}}>
            ...
            <p style={{color:'#64748b',fontSize:'0.95em',lineHeight:1.8,marginBottom:32}}>
              {activeCount} safe{activeCount!==1?'s':''} activo{activeCount!==1?'s':''} consultarán blockchain.
            ...
            <p style={{color:'#334155',fontSize:'0.75em',marginTop:12}}>Tiempo estimado: {activeCount*10}–{activeCount*15}s</p>
          </div>
        )}
          ...
          <div className="fade">
              ...
              return (
                <div>
                  ...
                  <div className="card" style={{padding:20,marginBottom:20}}>
                    <div style={{fontSize:'0.78em',color:'#94a3b8',textTransform:'uppercase',letterSpacing:'0.08em',marginBottom:16}}>Mix de Captación — EUR/USD: {EUR_USD}</div>
                    ...
                    <div style={{background:'#0f172a',borderRadius:8,padding:'14px 16px'}}>
                      <div style={{display:'flex',borderRadius:6,overflow:'hidden',height:28,marginBottom:10}}>
                        <div style={{background:'#818cf8',width:pct(pctFiat),display:'flex',alignItems:'center',justifyContent:'center',fontSize:'0.72em',fontWeight:700,color:'#0f172a'}}>{pctFiat>10?pct(pctFiat)+' Fiat':''}</div>
                        <div style={{background:'#fb923c',width:pct(pctCripto),display:'flex',alignItems:'center',justifyContent:'center',fontSize:'0.72em',fontWeight:700,color:'#0f172a'}}>{pctCripto>10?pct(pctCripto)+' Cripto':''}</div>
                      </div>
                  ...
                  <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(300px,1fr))',gap:12}}>
                    {data.map((safe,idx)=>{
                      ...
                      const grandTotal=fTotals.totalUsd+parseFloat(cTotals?.received||0);
                      return (
                        <div key={idx} className="card" style={{padding:20,borderTop:'3px solid '+bc,position:'relative',overflow:'hidden'}}>
                          ...
                          <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:14}}>
                            ...
                            <div style={{textAlign:'right'}}>
                              ...
                              <div style={{fontSize:'0.75em',color:'#475569'}}>{pct(grandTotal>0?(safeTotalUSD/grandTotal)*100:0)} del total</div>
                      ...
                      <div key={idx} className="card" style={{padding:20,borderTop:'3px solid '+(isSO?'#f59e0b':COLORS[idx]),position:'relative',overflow:'hidden'}}>
                        ...
                        {isSO?(
                          <div style={{background:'#f59e0b11',border:'1px solid #f59e0b33',borderRadius:8,padding:'10px 14px',fontSize:'0.78em',color:'#f59e0b88',textAlign:'center'}}>
                            🔒 Datos congelados · Cerrado {safe.frozenData?.date||''}
                          </div>
                          ...
                          <details>
                            ...
                            <div style={{marginTop:10,overflowX:'auto'}}>
                              <table style={{width:'100%',borderCollapse:'collapse',fontSize:'0.8em'}}>
                                ...
                                <tbody>{safe.transfers.slice(0,10).map((tx,i)=>(
            ...
            {activeTab==='fiat'&&(
              <div>
                ...
                <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(440px,1fr))',gap:12}}>
                  {data.map((safe,idx)=>{
                    const isSO=!!safe.soldOut,fiat=safe.fiat||{totalEur:0,totalUsd:0,count:0,txs:[]};
                    return (
                      <div key={idx} className="card" style={{padding:20,borderTop:'3px solid '+(isSO?'#f59e0b':COLORS[idx]),position:'relative',overflow:'hidden'}}>
                        {isSO&&<div className="so-overlay"/>}
                        ...
                        {isSO?(
                          <div style={{background:'#f59e0b11',border:'1px solid #f59e0b33',borderRadius:8,padding:'10px 14px',fontSize:'0.78em',color:'#f59e0b88',textAlign:'center'}}>
                            🔒 Datos congelados a {safe.frozenData?.date||''}
                          </div>
                        ):(fiat.txs||[]).length>0?(
                          <details>
            ...
            {activeTab==='2026'&&(()=>{
              ...
              data.forEach(safe=>{
                if(safe.soldOut&&safe.frozenData){cripto2026Total+=(safe.frozenData.criptoUsd2026!=null)?safe.frozenData.criptoUsd2026:parseFloat(safe.metrics.total||0);cripto2026Txs+=(safe.frozenData.criptoTxs2026!=null)?safe.frozenData.criptoTxs2026:(safe.metrics.count||0);}
                else{(safe.transfers||[]).forEach(tx=>{if(tx.timestamp.getFullYear()===2026){cripto2026Total+=tx.value||0;cripto2026Txs++;}});}
              });
              let fiat2026Total=0,fiat2026Count=0;
              data.forEach(safe=>{
                if(safe.soldOut&&safe.frozenData){fiat2026Total+=(safe.frozenData.fiatUsd2026!=null)?safe.frozenData.fiatUsd2026:(safe.fiat?.totalUsd||0);fiat2026Count+=(safe.frozenData.fiatCount2026!=null)?safe.frozenData.fiatCount2026:(safe.fiat?.count||0);}
                else{const sf=fiatData[safe.name];if(!sf)return;(sf.txs||[]).forEach(tx=>{if(getYearFromFecha(tx.fecha)==='2026'){fiat2026Total+=(tx.importe||0)*EUR_USD;fiat2026Count++;}});}
              });
              const total2026=cripto2026Total+fiat2026Total;
              const pctFiat2026=total2026>0?(fiat2026Total/total2026)*100:0,pctCripto2026=total2026>0?(cripto2026Total/total2026)*100:0;
              return (
                <div>
                  <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(200px,1fr))',gap:12,marginBottom:20}}>
